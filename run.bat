...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f demo-share
	call %MVN_EXEC% clean package -pl demo-share,demo-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill demo-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f demo-acs
	call %MVN_EXEC% clean package -pl demo-integration-tests,demo-platform,demo-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl demo-platform,demo-integration-tests,demo-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl demo-platform,demo-integration-tests
EXIT /B 0
:purge
